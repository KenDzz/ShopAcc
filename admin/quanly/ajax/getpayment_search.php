...
if ($username_check > 0) {
    $sql = "SELECT * FROM shop_log_payment INNER JOIN shop_user ON shop_log_payment.idUser=shop_user.id WHERE USER LIKE '%".$user."%'";
}else{
    $sql = "SELECT * FROM shop_log_payment INNER JOIN shop_user ON shop_log_payment.idUser=shop_user.id";
}
$paginationlink = "ajax/getpayment_search.php?page=";	
